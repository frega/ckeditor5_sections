...
`;var ve=n(60),fe=n.n(ve);class _e extends mt{static get properties(){return{from:{type:String},content:{type:String}}}connectedCallback(){super.connectedCallback(),this.content=this.innerHTML}render(){return B`
      <style>
        ${fe.a}
      </style>
      <div class="option" @click="${this.optionSelected}">
        <span class="option__label"
          >${_e.labels[this.from]}</span
        >
        <span class="option__content"><slot></slot></span>
      </div>
    `}optionSelected(){this.dispatchEvent(new CustomEvent("optionSelected",{detail:this.children.item(0)}))}}_e.labels={left:"Left version",right:"Right version",source:"Source version",empty:"Clear"},o.a.customElements.define("ck-button-option",_e);const ye=D`
<svg xmlns="http://www.w3.org/2000/svg" width="48" height="48" viewBox="0 0 48 48">
    <path d="M7.8 24c0-3.42 2.78-6.2 6.2-6.2h8V14h-8C8.48 14 4 18.48 4 24s4.48 10 10 10h8v-3.8h-8c-3.42 0-6.2-2.78-6.2-6.2zm8.2 2h16v-4H16v4zm18-12h-8v3.8h8c3.42 0 6.2 2.78 6.2 6.2s-2.78 6.2-6.2 6.2h-8V34h8c5.52 0 10-4.48 10-10s-4.48-10-10-10z"/>
</svg>
`;class xe extends ge{static get properties(){return{target:{type:String,attribute:"link-target"},error:Boolean,optionsElements:{type:String}}}resolveConflict(){this.isResolving=!0;const t=JSON.parse(this.getAttribute("left")),e=JSON.parse(this.getAttribute("right")),n=JSON.parse(this.getAttribute("source")),i=[];t&&i.push(xe.getItemInfo(t,"left")),e&&i.push(xe.getItemInfo(e,"right")),n&&i.push(xe.getItemInfo(n,"source")),this.optionsElements=i}static getItemInfo(t,e){return{label:e,href:t["link-target"],title:t.title,target:t.target,links:t["data-cta-type"]}}hasConflict(){return this.hasAttribute("left")||this.hasAttribute("source")||this.hasAttribute("right")}render(){const t=this.hasConflict()?this.resolveConflict:this.selectLink;return B`
      <div
        class="button ${this.target?"linked":"not-linked"} ${this.error?"error":""} ${this.hasConflict()?"test":""}"
      >
        <div class="button__content">
          <slot></slot>
        </div>
        <button @click="${this.inEditor?t:()=>{}}">
          ${ye}
        </button>
    ...
    `}resolved(t){const e=JSON.parse(this.getAttribute(t));this.modifyDocument(t=>{t.attributes(this,{"link-target":e["link-target"]}),t.removeAttribute(this,"left"),t.removeAttribute(this,"right"),t.removeAttribute(this,"source")}),this.isResolving=!1,this.optionsElements=[]}}xe.styles=pt`
  ${ge.styles}
  .button {
    position: relative;
  }
...
`,o.a.customElements.define("ck-button",xe);var ke=n(61),we=n.n(ke);class Se extends Zt{static get properties(){return{label:{type:String},resolved:{type:Boolean},isResolving:{type:Boolean}}}constructor(){super(),this.label=Se.label,this.resolved=!1,this.isResolving=!1}connectedCallback(){super.connectedCallback(),Array.from(this.children).forEach(t=>t.addEventListener("optionSelected",this.selectOptionHandler.bind(this))),document.addEventListener("click",t=>{!this.contains(t.target)&&this.isResolving&&(this.isResolving=!1)})}render(){return B`
    ...
    `}clickResolvingHandler(){this.isResolving=!0}selectOptionHandler(t){this.resolved=!0,this.isResolving=!1,this.modifyDocument(e=>e.swap(t.detail,this))}}Se.label="Conflict needs resolving";var Ee=n(62),Ce=n.n(Ee);o.a.customElements.define("ck-conflict-text",Se),o.a.customElements.define("ck-conflict-option",class extends mt{static get properties(){return{from:{type:String},content:{type:String}}}render(){return B`
      <style>
        ${Ce.a}
      </style>
      <div class="option" @click="${this.optionSelected}">
    ...
    `}firstUpdated(){"left"===this.position&&(this.style.width="50%",setTimeout(()=>{this.shadowRoot.querySelector(".media-conflict-option").style.minWidth=`${2*this.offsetWidth}px`},100))}optionSelected(){this.dispatchEvent(new CustomEvent("optionSelected",{detail:this.children.item(0)}))}});var ze=n(65),Le=n.n(ze);const Me=D`
...
`;o.a.customElements.define("ck-media",class extends Zt{static get properties(){return{loaderIsVisible:Boolean,mediaType:{attribute:"data-media-type",type:String},mediaUuid:{attribute:"data-media-uuid",type:String},mediaDisplay:{attribute:"data-media-display",type:String},enableUpload:{attribute:"ck-upload",type:Boolean},enableEdit:{attribute:"ck-edit",type:Boolean},enableReset:{type:Boolean},buttonPosition:{attribute:"ck-button-position",type:String},preview:String,error:Boolean}}validate(){const t=this.error;this.error=!this.mediaUuid,!t&&this.error?this.emitElementValidationErrorEvent("Media is required","media_required"):t&&!this.error&&this.emitElementValidationErrorResolvedEvent()}connectedCallback(){super.connectedCallback(),this.requestInformation("show-errors",{},t=>{t&&this.validate()}),this.enableReset=!this.getAttribute("ck-validation")&&!!this.mediaUuid}renderPreview(){this.loaderIsVisible=!0,this.requestInformation("media-preview",{type:this.mediaType,uuid:this.mediaUuid,display:this.mediaDisplay},t=>{this.preview=t,this.loaderIsVisible=!1})}updated(t){this.previewPane=this.shadowRoot.querySelector(".ck-media__preview"),t.has("mediaUuid")&&this.mediaUuid&&(this.validate(),this.renderPreview()),t.has("preview")&&this.preview&&(this.previewPane.innerHTML=this.preview),this.enableReset=!this.getAttribute("ck-validation")&&!!this.mediaUuid}render(){return B`
      <style>
        ${Le.a}
      </style>
    ...
    `}}Re.initializeWithErrors=!1,o.a.customElements.define("ck-textfield",Re);var Ue=n(67),je=n.n(Ue);o.a.customElements.define("ck-collapse",class extends te{static get properties(){return{collapseIsOpen:{type:Boolean}}}connectedCallback(){super.connectedCallback(),null==this.collapseIsOpen&&(this.collapseIsOpen=!1)}toggleCollapse(){this.collapseIsOpen=!this.collapseIsOpen}render(){return B`
      <style>
        ${je.a}
      </style>
