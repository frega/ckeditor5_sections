...
`;o.a.customElements.define("ck-media",class extends Ot{static get properties(){return{loaderIsVisible:Boolean,mediaType:{attribute:"data-media-type",type:String},mediaUuid:{attribute:"data-media-uuid",type:String},mediaDisplay:{attribute:"data-media-display",type:String},enableUpload:{attribute:"ck-upload",type:Boolean},enableEdit:{attribute:"ck-edit",type:Boolean},buttonPosition:{attribute:"ck-button-position",type:String},preview:String}}connectedCallback(){super.connectedCallback()}renderPreview(){this.loaderIsVisible=!0,this.requestInformation("media-preview",{type:this.mediaType,uuid:this.mediaUuid,display:this.mediaDisplay},t=>{this.preview=t,this.loaderIsVisible=!1})}updated(t){this.previewPane=this.shadowRoot.querySelector(".ck-media__preview"),t.has("mediaUuid")&&this.mediaUuid&&this.renderPreview(),t.has("preview")&&this.preview&&(this.previewPane.innerHTML=this.preview)}render(){return L`
      <style>
        ${fe.a}
      </style>
      <div class="ck-media">
