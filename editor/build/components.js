...
`,o.a.customElements.define("ck-section",Ut);var jt=n(48),Ft=n.n(jt),qt=n(49),Zt=n.n(qt),Wt=n(50),Jt=n.n(Wt),Xt=n(51),Yt=n.n(Xt);o.a.customElements.define("ck-gallery",class extends Ot{static get properties(){return{items:Array,currentItem:{type:Number,attribute:"ck-current-item"},numberOfChildren:{type:Number},maxItems:{type:Number,attribute:"ck-max"},sections:{type:String,attribute:"ck-contains"}}}constructor(){super(),this.items=[],this.currentItem=0}connectedCallback(){super.connectedCallback(),this.shadowRoot.addEventListener("slotchange",()=>{this.numberOfChildren=Array.from(this.children).filter(t=>"BR"!==t.nodeName).length,this.items=Array.from(this.children).filter(t=>"BR"!==t.nodeName).map((t,e)=>({title:e+1,index:e}))}),this.numberOfChildren=Array.from(this.children).filter(t=>"BR"!==t.nodeName).length,this.maxItems=this.maxItems||0,this.items=Array.from(this.children).filter(t=>"BR"!==t.nodeName).map((t,e)=>({title:e+1,index:e})),this.setGalleryItem(this.currentItem)}render(){return L`
      <style>
        ${Ft.a}
      </style>
...
`,o.a.customElements.define("ck-button",oe);var re=n(55),se=n.n(re);class ae extends Ot{static get properties(){return{label:{type:String},resolved:{type:Boolean},isResolving:{type:Boolean}}}constructor(){super(),this.label=ae.label,this.resolved=!1,this.isResolving=!1}connectedCallback(){super.connectedCallback(),Array.from(this.children).forEach(t=>t.addEventListener("optionSelected",this.selectOptionHandler.bind(this))),document.addEventListener("click",t=>{!this.contains(t.target)&&this.isResolving&&(this.isResolving=!1)})}render(){return L`
    ...
    `}clickResolvingHandler(){this.isResolving=!0}selectOptionHandler(t){this.resolved=!0,this.isResolving=!1,this.modifyDocument(e=>e.swap(t.detail,this))}}ae.label="Conflict needs resolving";var le=n(56),ce=n.n(le);class de extends pt{static get properties(){return{from:{type:String},content:{type:String}}}connectedCallback(){super.connectedCallback(),this.content=this.innerText}render(){return L`
      <style>
        ${ce.a}
      </style>
